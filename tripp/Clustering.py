...
class Clustering:
    ...
    output_directory: str
        Directory of the output files to be saved.
    output_prefix: str
        Prefix of the output files to be saved.
    buriedness_file: str or list, default=None
        Path to the CSV file containing the buriedness values generated by the
        Trajectory class using the same trajectory_file and topology_file selected for the
        clustering. When list, it is a list of paths of the CSV files corresponding to the
        dictionary selected for trajectory_file. When selecting a list of files for the
        buriedness_file argument the order of the files needs to be the same as in the
        trajectory_file dictionary. If None, the buriedness values are not used for clustering.
    include_distances: bool, default=False
        If True, the relative positions (as distances between the charge centers) between the 
        selections are used as additional features for the clustering alongside the pKa values.
    include_buriedness: bool, default=False
        If True, the buried ratio (buriedness) of the selections are used as additional features
    ...
    dimensionality_reduction: bool, befault=False
        ...
        selected charge center distances/buriedness) using PCA. The number of components 
        is selected that described >=90% of the cummulative variance and the clustering 
        is done on those components.
    """
    ...
    def kmedoids(
        ...
        automatic: bool, default=False
            If True, the clustering is run using various cluster numbers 
            (defined by max_clusters). The silhouette score is determined 
            at each iteration and the best number of clusters is chosen 
            based on the highest silhouette score.

        n_clusters: int, default=8
            Number of clusters used for the clustering. Ignored if 
            automatic=True.
    ...
    def greedy(
        ...
        automatic: bool, default=False
            If True, the clustering is run using various cutoffs (number of cutoffs
            defined by max_cutoffs). The silhouette score is determined at each
            iteration and the best cutoff is chosen based on the highest silhouette
            score. Cutoffs that produce more than the allowed maximum number of clusters
            (defined by max_clusters) or that produce only one cluster are excluded.
        
        max_cluster_population: float, default=0.95
            Used when automatic=True. The maximum population of the most
            populated cluster allowed. If the most populated cluster contains 
            more than this, the cutoff is skipped.
        
        cutoff: float, default=0.1
            Cutoff used for clustering. Ignored if automatic=True.

        max_clusters: int, default=20
            Maximum number of clusters allowed when automatic=True.

        max_cutoffs: int, default=20
            Maximum RMSD cutoff used when automatic=True.
        """
        ...
        if automatic == False:
            ...
            for cutoff_i in np.arange(step, max_rmsd + step, step):
                ...
                if len(labels) == len(set(labels)):
                    print(
                        f"Cutoff of {cutoff_i} produces no clusters. Moving on to next cutoff"
                    )

                elif len(set(labels)) == 1:
                    print(
                        f"Cutoff of {cutoff_i} produces only one cluster. Moving on to next cutoff."
                    )

                elif len(set(labels)) > max_clusters:
                    print(
                        f"Cutoff of {cutoff_i} produces more than {max_clusters} clusters. Moving on to next cutoff."
                    )
    ...
    def dbscan(
        ...
        automatic: bool, default=False
            If True, the clustering is run using various combinations of eps and 
            min_samples values. The silhouette score is determined at each iteration and
            the best combination of values is chosen based on the highest
            silhouette score. Combinations that produce more than the allowed maximum 
            number of clusters (defined by max_clusters) or that produce only one cluster 
            are excluded.
        

        eps_range: tuple, default=(0.005, 0.505, 0.005)
            The range of epsilon value screened in the automatic grid search. Value is
            generated with np.arange(start, stop, step).

        min_samples_range: tuple, default=(2, 11, 1)
            The range of min_samples value screend in the automatic grid search. Value
            ...
            for eps in np.arange(eps_range[0], eps_range[1], eps_range[2]):
                ...
                for min_samples in np.arange(
                    ...
                    if len(set(labels)) == len(labels):
                        ...
                        print(
                            f"Clustering with parameters eps={eps} and min_samples={min_samples} results in the most populated cluster containing more than {max_cluster_population*100}% of frames. Moving on to next st of parameters."
                        )
    ...
    def hdbscan(
        ...
        **kwargs):
        ...
        automatic: bool, default=False
            ...
            min_samples values. The silhouette score is determined at each iteration and
            the best combination of values is chosen based on the highest silhouette score.
            Combinations that produce more than the allowed maximum number of clusters
            (defined by max_clusters) or that produce only one cluster are excluded.
        ...
        min_cluster_size_range: tuple, default=(20, 80, 5)
            The range of minimum cluster size screened in the automatic grid search. Value 
            is generated with np.arange(start, stop, step).
